/**
 * Verification tests for the PURE / DETERMINISTIC parts of `dialog`.
 *
 * Scope:
 *   - registry/remocn-ui/dialog/index.tsx  — DialogState union membership,
 *     dialogStyle presets, dialogStyleContext
 *   - registry/remocn-ui/dialog/config.ts  — dialogConfig.controls wiring
 *     + dialogConfig.snippet output (the state → JSX codegen)
 *   - registry/remocn-ui/dialog/use-dialog-transition.ts
 *     — tweenDialogStyle interpolation, DEFAULT_DURATION
 *
 * The render path (index.tsx) is a PURE-STATE model: `(state) => visual`.
 * `<Dialog>` reads `useRemocnTheme()` internally — that hook is pure at
 ...
 * IMPORT STRATEGY
 * --------------------------------------------------------------------------
 * `config.ts` imports `DialogState` from `@/registry/remocn-ui/dialog`
 * and the pieces under test never CALL a Remotion runtime API at import time —
 * `dialogConfig` is a plain object; `.snippet` is a pure string builder;
 * `dialogStyle` and `tweenDialogStyle` are pure value functions.
...
/**
 * The DialogState union, enumerated as a runtime list for membership checks.
 * Must stay in sync with `export type DialogState` in index.tsx.
 */
...
/**
 * A shared DialogStyleContext built from the default light theme.
 * `dialogStyleContext` takes only `(theme)` — no variant arg, unlike accordion.
 */
const ctx = dialogStyleContext(defaultLightTheme);

// ===========================================================================
// 1. DialogState union membership
// ===========================================================================
...
// ===========================================================================
// 2. dialogConfig.controls.state — customizer control wiring
// ===========================================================================
...
// ===========================================================================
// 3. dialogConfig.snippet — pure string builder
...
// ===========================================================================
// 4. dialogStyleContext — derives concrete colors from the theme.
...
// ===========================================================================
// 5. dialogStyle presets — pure (state, ctx) => DialogStyle
//    dialogStyleContext and dialogStyle are exported and frame-free.
...
// ===========================================================================
// 6. tweenDialogStyle — pure linear interpolation between two DialogStyles.
