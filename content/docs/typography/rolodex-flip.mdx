...
`slot-machine-roll` and `number-wheel` are numeric reels, `per-word-crossfade` is a flat opacity swap — reach for `RolodexFlip` when arbitrary strings should cycle with physical depth at a fixed width.
